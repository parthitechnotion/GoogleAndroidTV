...
public class StreamingDrmSessionManager implements DrmSessionManager {
  ...
  public StreamingDrmSessionManager(UUID uuid, Looper playbackLooper, MediaDrmCallback callback,
    ...
    mediaDrmHandler = new MediaDrmHandler(playbackLooper, this);
    postResponseHandler = new PostResponseHandler(playbackLooper, this);
    state = STATE_CLOSED;
  }
  ...
  public void open(DrmInitData drmInitData) {
    ...
    if (postRequestHandler == null) {
      ...
      postRequestHandler = new PostRequestHandler(requestHandlerThread.getLooper(), this);
    }
  ...
  private static class MediaDrmHandler extends WeakHandler<StreamingDrmSessionManager> {
    public MediaDrmHandler(Looper looper, StreamingDrmSessionManager ref) {
      super(looper, ref);
    }

    @Override
    public void handleMessage(Message msg, StreamingDrmSessionManager sessionManager) {
      if (sessionManager.openCount == 0 || (sessionManager.state != STATE_OPENED
              && sessionManager.state != STATE_OPENED_WITH_KEYS)) {
        return;
      }
      switch (msg.what) {
        case MediaDrm.EVENT_KEY_REQUIRED:
          sessionManager.postKeyRequest();
          return;
        case MediaDrm.EVENT_KEY_EXPIRED:
          sessionManager.state = STATE_OPENED;
          sessionManager.onError(new KeysExpiredException());
          return;
        case MediaDrm.EVENT_PROVISION_REQUIRED:
          sessionManager.state = STATE_OPENED;
          sessionManager.postProvisionRequest();
          return;
      }
    }
  ...
  private static class PostResponseHandler extends WeakHandler<StreamingDrmSessionManager> {
    public PostResponseHandler(Looper looper, StreamingDrmSessionManager ref) {
      super(looper, ref);
    }
    ...
    public void handleMessage(Message msg, StreamingDrmSessionManager sessionManager) {
      switch (msg.what) {
        case MSG_PROVISION:
          sessionManager.onProvisionResponse(msg.obj);
          return;
        case MSG_KEYS:
          sessionManager.onKeyResponse(msg.obj);
          return;
      }
    }
  ...
  private static class PostRequestHandler extends WeakHandler<StreamingDrmSessionManager> {


    public PostRequestHandler(Looper looper, StreamingDrmSessionManager ref) {
      super(looper, ref);
    }
    ...
    public void handleMessage(Message msg, StreamingDrmSessionManager sessionManager) {
      ...
      try {
        switch (msg.what) {
          case MSG_PROVISION:
            response = sessionManager.callback
                    .executeProvisionRequest(sessionManager.uuid, (ProvisionRequest) msg.obj);
            break;
          case MSG_KEYS:
            response = sessionManager.callback
                    .executeKeyRequest(sessionManager.uuid, (KeyRequest) msg.obj);
            break;
          default:
      ...
      sessionManager.postResponseHandler.obtainMessage(msg.what, response).sendToTarget();
    }
