...
public final class AudioTrack {
  ...
  // For correct frame calculation regardless of sample size change.
  private class FrameCounter {
    public static final int FRAMES_PER_AC3_SAMPLE = 1536;

    // Keep submitted bytes according to sample size for AC3 frame calculation.
    private final SparseLongArray submittedBytes;

    private long totalSubmittedBytes;
    private int sampleSize;

    private FrameCounter() {
      submittedBytes = new SparseLongArray();
    }

    private void reset() {
      totalSubmittedBytes = 0;
      sampleSize = 0;
      submittedBytes.clear();
    }

    // Set size of the current sample which is ready for submission to framework AudioTrack.
    private void setSampleSize(int bytes) {
      sampleSize = bytes;
    }

    // Specified bytes portion of sample is submitted to framework AudioTrack.
    private void submit(int bytes) {
      if (isAc3) {
        submittedBytes.put(sampleSize, submittedBytes.get(sampleSize) + bytes);
      }
      totalSubmittedBytes += bytes;
    }

    private long submittedBytes() {
      return totalSubmittedBytes;
    }
    ...
    private long submittedFrames() {
      if (isAc3) {
        long ret = 0;
        for (int i = 0; i < submittedBytes.size(); ++i) {
          if (submittedBytes.keyAt(i) != 0) {
            ret += submittedBytes.valueAt(i) * FRAMES_PER_AC3_SAMPLE / submittedBytes.keyAt(i);
          }
        }
        return ret;
      }
      return bytesToFrames(totalSubmittedBytes);
    }

    private long sampleToFrames(int size) {
      if (!isAc3) {
        return bytesToFrames(size);
      }
      return FrameCounter.FRAMES_PER_AC3_SAMPLE;
    }

    private long bufferToFrames(long size) {
      if (!isAc3) {
        return bytesToFrames(size);
      }
      // TODO: Calculate buffer frame size according to the median sample size.
      return sampleSize == 0 ? 0L : size * FRAMES_PER_AC3_SAMPLE / sampleSize;
    }
  }
  ...
  private static final long MIN_BUFFER_DURATION_US = 130000;
  /** A maximum length for the {@link android.media.AudioTrack} buffer, in microseconds. */
  private static final long MAX_BUFFER_DURATION_US = 490000;
  /**
  ...
  public int handleBuffer(ByteBuffer buffer, int offset, int size, long presentationTimeUs)
    ...
    if (temporaryBufferSize == 0) {
      frameCounter.setSampleSize(size);

      // This is the first time we've seen this {@code buffer}.
      // Note: presentationTimeUs corresponds to the end of the sample, not the start.
      long bufferStartTime = presentationTimeUs
          - framesToDurationUs(frameCounter.sampleToFrames(size));
      if (startMediaTimeState == START_NOT_SET) {
        ...
        long expectedBufferStartTime = startMediaTimeUs
            + framesToDurationUs(frameCounter.submittedFrames());
        if (startMediaTimeState == START_IN_SYNC
    ...
    if (Util.SDK_INT < 21) {
      ...
      int bytesPending =
          (int) (frameCounter.submittedBytes()
              - (audioTrackUtil.getPlaybackHeadPosition() * frameSize));
    ...
    frameCounter.submit(bytesWritten);
    if (temporaryBufferSize == 0) {
  ...
  public boolean hasPendingData() {
    return isInitialized()
        && (frameCounter.submittedFrames() > audioTrackUtil.getPlaybackHeadPosition()
  ...
  public boolean hasEnoughDataToBeginPlayback() {
    ...
    return frameCounter.submittedBytes() > (minBufferSize * 3) / 2;
  }
      ...
      if (getLatencyMethod != null) {
        try {
          ...
          latencyUs = (Integer) getLatencyMethod.invoke(audioTrack, (Object[]) null) * 1000L
              - framesToDurationUs(frameCounter.bufferToFrames(bufferSize));
  ...
  private static class AudioTrackUtil {
    ...
    public long getPlaybackHeadPosition() {
      ...
      if (lastRawPlaybackHeadPosition > rawPlaybackHeadPosition) {
        ...
        if (isPassthrough && lastRawPlaybackHeadPosition < (1L << 32) - sampleRate * 5L) {
          Log.e(TAG, "audio_position playhead jump: "
              + (lastRawPlaybackHeadPosition - rawPlaybackHeadPosition));
        } else {
          Log.i(TAG, "audio_position playhead is wrapped");
